...
| Encoder caps at ~8 FPS at 1280×1280 regardless of target FPS, encoder input queue at the eviction threshold | Bulk JNI byte[] marshalling per frame — regression of the direct-`ByteBuffer` path in `HardwareH265Encoder` | Visible frame loss |
...
- **Direct `ByteBuffer` I/O on the H.265 encoder** in [`HardwareH265Encoder.cs`](Assets/Scripts/Encoding/HardwareH265Encoder.cs). MediaCodec's input/output are `java.nio.ByteBuffer`s backed by direct (native) memory. The original implementation used `AndroidJavaObject.Call("put", sbyte[])` for input and a `NewByteArray` → `ByteBuffer.get(byte[])` → `FromSByteArray` round-trip for output — both routes Unity's JNI byte-array bulk-copy. At 1280×1280 NV12 the input alone is **2.4 MB/frame**, and the marshalling cost dominated the encoder loop, capping throughput at ~8 FPS regardless of the requested rate. With capture running at 14–24 FPS, the input queue saturated at its 5-deep eviction threshold and ~⅓ of frames were silently dropped (the older [Pipeline] log showed `cap=14.4fps enc=8.3fps sent=8.3fps`). The current path calls `getInputBuffer(idx)` (the API-21+ replacement for the deprecated `getInputBuffers()`), grabs the buffer's native address via `AndroidJNI.GetDirectBufferAddress`, and writes/reads through `Marshal.Copy` — a single native memcpy with no JNI byte-array allocation. Output side mirrors the same pattern. Encoder is now capture-bound at the highest tested rates (~24 FPS at the camera's hardware ceiling). `AndroidJNI.GetDirectBufferAddress` returns `sbyte*` on the project's Unity version, so the calls live inside `unsafe` blocks; `-unsafe` is enabled project-wide via [`Assets/csc.rsp`](Assets/csc.rsp).
